...
export function addDefaultHeaders(response: Response) {
  ...
  response.headers.set(
    ...
    `default-src 'none'; script-src 'self' 'unsafe-eval' cdn.ampproject.org ajax.cloudflare.com static.cloudflareinsights.com *.algolia.net *.algolianet.com mc.yandex.ru www.googletagmanager.com; style-src 'self' 'unsafe-inline'; img-src 'self' www.googletagmanager.com blog-images.clickhouse.tech data: mc.yandex.ru; object-src 'self' blog-images.clickhouse.tech; connect-src 'self' mc.yandex.ru cdn.ampproject.org *.algolia.net *.algolianet.com *.ingest.sentry.io hn.algolia.com www.reddit.com; child-src blob: mc.yandex.ru; frame-src blob: mc.yandex.ru www.youtube.com datalens.yandex blog-images.clickhouse.tech; font-src 'self' data:; base-uri 'none'; form-action 'self'; frame-ancestors webvisor.com metrika.yandex.ru; prefetch-src 'self'`,
  );
